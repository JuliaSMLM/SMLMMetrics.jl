"""
    efficiency(a::Array{<:Real}, b::Array{<:Real}, cutoff::Vector{<:Real}, α::Vector{<:Real}=nothing)
...
``E = 100 - \\sqrt{(100 - JI)^2 + α^2 * RMSE^2}``
...
Detection accuracy is expressed in units of 0 to 100%. 
The efficiency ranges up to 100% for a perfect fitting algorithm.
"""
function efficiency(a::Array{<:Real}, b::Array{<:Real}, cutoff::Vector{<:Real}, α::Vector{<:Real}=nothing)
    ...
    E_lateral = 100 - sqrt((100 - ji)^2 + α[1]^2 * lateral_rmse^2)
    E_axial = size(a, 1) == 3 ? (100 - sqrt((100 - ji)^2 + α[3]^2 * axial_rmse^2)) : 0.0
    E = (size(a, 1) == 3 ? (E_lateral + E_axial) / 2 : E_lateral)
